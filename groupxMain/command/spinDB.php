...
// Query the database.
$str_query = 'SELECT * FROM '.$eventtable.' WHERE time_start <= DATE_ADD(CURTIME() + interval +0 hour, INTERVAL 1 HOUR) and time_end >= DATE_ADD(CURTIME() + interval +0 hour, INTERVAL 1 HOUR) and date_start = CURDATE()';  
if (!$str_query) {
